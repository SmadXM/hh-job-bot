...
async def update_vacancy_status(db_path: str, vacancy_id: str, status: str) -> None:
    async with aiosqlite.connect(db_path) as db:
        await db.execute(
            "UPDATE vacancies SET status = ? WHERE vacancy_id = ?",
            (status, vacancy_id),
        )
        await db.commit()
    ...
    async with aiosqlite.connect(db_path) as db:
        cur = await db.execute(
            ...
            SELECT
                ...
                SUM(CASE WHEN status = 'not_interested' THEN 1 ELSE 0 END) AS skipped
            FROM vacancies
        ...
        return {
            "total":      row[0] or 0,
            "interested": row[1] or 0,
            "pending":    row[2] or 0,
            "skipped":    row[3] or 0,
        }


async def list_interesting(db_path: str, limit: int = 20) -> list[dict]:
    async with aiosqlite.connect(db_path) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            """
            SELECT vacancy_id, title, company, salary_from, salary_to, currency,
                   city, schedule, url, fit_score
              FROM vacancies
